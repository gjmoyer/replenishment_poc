...
def test_new_zero_appended_to_existing_pick():
    pick, seen = merge_newcomers(["eggs-12ct-005"], ["eggs-12ct-005"],
                                 ["eggs-12ct-005", "soda-12pk-101"], ALL)
...
def test_manual_pick_preserved_no_touch():
    pick, seen = merge_newcomers(["milk-1gal-001"], ["eggs-12ct-005"],
                                 ["eggs-12ct-005"], ALL)
    assert pick is None
    assert seen == ["eggs-12ct-005"]
...
def test_cleared_picker_rearms_on_change():
    ...
    pick, _ = merge_newcomers([], ["eggs-12ct-005"],
                              ["eggs-12ct-005", "milk-1gal-001"], ALL)
